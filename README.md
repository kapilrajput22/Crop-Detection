...
```python
data_dir = r'/home/kapil.rajput/Downloads/weed-detection-in-soybean-crops/dataset/'
...
stats = net.train(X_dev, y_dev, X_val, y_val, 
              ...
              reg=0.0, num_iters=100, 
              batch_size=100, it_verbose = 10,verbose=True)
```

    iteration: 0 / 100 | Loss: 1.395131
    iteration: 10 / 100 | Loss: 1.326269
    iteration: 20 / 100 | Loss: 1.273000
    iteration: 30 / 100 | Loss: 1.255074
    iteration: 40 / 100 | Loss: 1.234228
    iteration: 50 / 100 | Loss: 1.158207
    iteration: 60 / 100 | Loss: 1.186648
    iteration: 70 / 100 | Loss: 1.143848
    iteration: 80 / 100 | Loss: 1.120775
    iteration: 90 / 100 | Loss: 1.148029
...
```
    ...
    iteration: 0 / 500 | Loss: 1.423242
    iteration: 100 / 500 | Loss: 0.691731
    iteration: 200 / 500 | Loss: 0.554019
    iteration: 300 / 500 | Loss: 0.527078
    iteration: 400 / 500 | Loss: 0.519133
    hs: 200 learn: 7.6e-05 reg 0.0 iter 500 train-acc: 0.84 val_acc 0.77
...
```
    ...
    ([<matplotlib.axis.XTick at 0x7f4c6265e9b0>,
      <matplotlib.axis.XTick at 0x7f4c60391f28>,
      <matplotlib.axis.XTick at 0x7f4c60391898>,
      <matplotlib.axis.XTick at 0x7f4c502f0ac8>],
...
```python
# Predicting From X_Test Data form 200 image sample
# random sample from 0 to 199
i =100
predictions = best_net.predict(X_test)[i]
if predictions == 0:
    a=="Broad Leaf"
elif predictions == 1:
    a="Grass"
...
plt.imshow(X_test[i].reshape(im_width, im_height, im_channel).astype('uint8'))
plt.title("Predicted As: " + a)
predictions = best_net.predict(X_test)[i]
